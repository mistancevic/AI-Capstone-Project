...
Phase 1 fails closed. The exclusion list keeps out anyone with a disclosed eating-disorder history, but S1 to S7 exist for the undisclosed signal in someone nobody flagged - and phase 1 runs that screening at the sensitivity I measured: zero of ten on wording it had never seen. Exclusions and screening protect against different people, and one cannot stand in for the other. So for phase 1, anything the rules cannot confidently tier does not proceed: the client gets a clarifying question and I get a flag, rather than a card. I accept the higher false-positive rate as the price of the weaker net. Three clients I speak to daily can absorb being over-flagged; they cannot absorb a miss. The fail-safe is wide because the net is weak, and it narrows again in phase 2 when the leg that caught ten of ten is switched on.
...
The false positives I am choosing to cause. Phase 1 fails closed, so anything the rules cannot confidently tier becomes a clarifying question and a flag rather than an answer. That will flag clients who were fine. It is a deliberate trade - a false alarm costs one awkward message to someone I speak to daily, a miss costs someone in trouble receiving a meal card - and it is the fourth thing I count each day: flags I raised that I judged unwarranted afterwards. Those counts are the over-refusal baseline I have never had, produced by the pilot rather than waited for.
...
Phase 1 - the pilot. Three clients, four weeks, deterministic path only, failing closed - anything the rules cannot confidently tier goes to a clarifying question and a coach flag rather than an answer - with the exclusions, the frozen safety policy and the pause conditions set out above. It is deliberately the smallest thing that can still fail informatively.
