...
| **Ask only when the answer is load-bearing** — today the app clarifies whenever a required slot is empty, regardless of whether filling it would change the recommendation. Three moves, in order of preference: (a) *source* a missing figure instead of requesting it — offer two or three close rows from `foods.csv` ("cake slice ≈ 350 kcal") so the client taps rather than types, and the card cites the row it used; (b) compute the card across a plausible range and, when the top option is the same at both ends, **do not ask at all** — say so on the card ("whether that cake was 200 or 500, tonight's answer is the same"), which is not a guess but a demonstration that the guess is irrelevant; (c) when it genuinely changes the answer, state what changes in one sentence rather than posing a question. Same posture A5 already takes toward imperfection — show the honest range instead of demanding precision | User, Deploy walkthrough 2026-07-31, on the clarify chain: *"I see it as the purpose of the app to save such day for our client."* Grounded in `strategy-notes.md`, which lists *"logging burden and guilt"* under **eliminated** — "how many calories was the cake?" is that burden returning through the clarify door, in the one moment the client is already having a bad day | Adds real complexity: (b) means computing the card N times and comparing recommendations, and the "same answer" test needs defining (top option only, or the whole ranking?). Revive with the free-text intake, not before — the two are the same feature seen from opposite ends, and a multi-turn text clarify is where an unnecessary question actually costs something. Until then the demo carries a single scripted round, which is enough to show the loop and does not need this |
| **Meal-slot naming — separate the join key from the client's words** — `meal_slot` earns its four literals as *data*: it joins `plans.csv` ↔ `disruptions.csv` (`meal_to_solve`, `eaten_as_planned`, `displaced_meals`) ↔ `foods.csv` (`meal_types`, which is what stops oats being offered for dinner), and `computeCard` sorts every plan meal into eaten / reserved / the gap by that name. What it has not earned is being the wording shown to a client — "the 15:00 one, 350 kcal and 30 g" says the same thing without the jargon. The deeper half is not display: brunch, dessert, late-night, two-meal days and one-meal-a-day mean a plan is *n* slots the coach names, not four fixed ones, and `foods.csv` would have to key off slot *properties* (time of day, size) instead of four literals | User, Deploy walkthrough 2026-07-31, on the p34 clarify presets: *"meal can be brunch as well… some people are having only two meals per day, or even only one"* | Not now — it would change working arithmetic for the sake of wording. The display half can ship alone and cheaply, once there is a reason. The variable-slot half waits for a real coach whose plan does not fit four slots, and ships with the strategic-vs-punitive skip work below: OMAD and IF clients are the same cohort, and both items are about the app not assuming a four-meal day is the only healthy shape |
...
| **Fail-closed screening for the pilot** — when the deterministic rules cannot confidently tell what a message is about, the app must not answer it: the client gets a clarifying question, the coach gets a non-urgent flag, and no card is built. Four pieces: (a) a second question in the screen — not "is there a danger word?" but "do I recognise this situation?", answered from the trigger/slot extraction that already returns `unknown`; (b) a wider *worth-a-second-look* word list, separate from the stop words — tired, weak, shaky, off, not myself, since Monday, all week, barely, black coffee, lightheaded, wobbly, empty; (c) unrecognised **and** on that list → clarify **plus** a coach flag, where today an unclear message only gets a question; (d) count every time it fires, so the pilot produces the over-refusal baseline that does not exist today | Faculty review of the Deploy rows, 2026-08-07: the exclusion list only removes clients who have *disclosed* an eating-disorder history, but S1–S7 exist for the person who has told nobody — and that screening measured 0/10 on unseen wording. Exclusions and screening protect different people; neither substitutes for the other. *"Three clients you speak to daily can absorb over-flagging. They cannot absorb a miss."* | **Phase 1 of the pilot, and not before** — it is pointless without real clients typing real messages, which needs the free-text intake below and the runtime counters. Deliberately temporary: the fail-safe is wide because the net is weak, and it narrows again once the model leg (10/10 on the same test) is switched on in phase 2 |
